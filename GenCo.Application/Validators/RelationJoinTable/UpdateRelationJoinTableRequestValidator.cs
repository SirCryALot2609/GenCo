...
public class UpdateRelationJoinTableRequestValidator : AbstractValidator<UpdateRelationJoinTableRequestDto>
{
    private readonly IGenericRepository<Domain.Entities.RelationJoinTable> _repository;

    public UpdateRelationJoinTableRequestValidator(IGenericRepository<Domain.Entities.RelationJoinTable> repository)
    {
    ...
    private async Task<bool> BeUniqueName(UpdateRelationJoinTableRequestDto dto, string joinTableName, CancellationToken ct)
    {
        var spec = new RelationJoinTableByNameSpec(dto.RelationId, joinTableName);
        var tables = await _repository.FindAsync(spec, cancellationToken: ct);
        return tables.All(t => t.Id == dto.Id); // cho phép trùng tên nếu cùng Id
    }
}
